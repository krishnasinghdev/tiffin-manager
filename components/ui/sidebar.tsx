...
const MobileSidebarTrigger = React.forwardRef<React.ElementRef<typeof Button>, React.ComponentProps<typeof Button>>(
  ({ onClick, ...props }, ref) => {
    const { toggleSidebar } = useSidebar()
    ...
    return (
      <button
        ...
        data-sidebar="trigger"
        className="group bg-primary/25 flex flex-col items-center justify-center px-4"
      >
        <Icons.PanelLeftOpen className="text-muted-foreground group-hover:text-foreground mb-1 h-6 w-6" />
        <span className="text-muted-foreground group-hover:text-foreground text-xs">Menu</span>
      </button>
    )
  }
)
